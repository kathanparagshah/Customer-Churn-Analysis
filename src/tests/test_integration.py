...
class TestEndToEndPipeline:
    ...
    def sample_raw_data(self, temp_project_dir):
        ...
        data = pd.DataFrame(
            {
                ...
                "EstimatedSalary": np.random.uniform(
                    11.58, 199992.48, n_samples
                ),
        ...
        # Save raw data
        raw_data_path = (
            temp_project_dir / "data" / "raw" / "Churn_Modelling.csv"
        )
    ...
    def test_complete_pipeline(self, temp_project_dir, sample_raw_data):
        ...
        validation_success = loader.validate_data(
            raw_data
        )  # This already calls save_interim_data
        assert validation_success

        interim_path = (
            temp_project_dir / "data" / "interim" / "churn_raw.parquet"
        )
        ...
        processed_path = (
            temp_project_dir / "data" / "processed" / "churn_cleaned.parquet"
        )
        ...
        feature_importance = engineer.calculate_feature_importance(
            featured_data
        )
        ...
        features_path = (
            temp_project_dir / "data" / "processed" / "churn_features.parquet"
        )
        ...
        cols = list(segmentation_data.columns)
        print(f"Available columns for segmentation: {cols}")
        ...
        segment_profiles = segmentation.analyze_segments(
            segmentation_data, features, labels
        )
        ...
        print(f"✓ Baseline model trained (Accuracy: {acc:.3f})")
        ...
        predictor.evaluation_results[best_model_name] = {
            "accuracy": 0.8,
            "roc_auc": 0.8,
        }
        ...
        model_path = (
            temp_project_dir / "models" / f"{best_model_name}_model.pkl"
        )
        ...
        data = explainer.load_data(str(data_path), sample_size=100)
        ...
        try:
            explainer.create_explainer(
                X_explain.sample(min(50, len(X_explain)))
            )
        ...
        best_acc = predictor.evaluation_results[best_model_name]["accuracy"]
...
class TestDataPipelineIntegration:
    ...
    def test_data_flow_consistency(self, temp_dir):
        ...
        pipeline, processed_data = cleaner.create_preprocessing_pipeline(
            clean_data
        )
        ...
        print(f"Processed data columns ({len(cols)}): {cols}")
        featured_data = engineer.create_ratio_features(processed_data)
        cols = list(featured_data.columns)
        print(f"After ratio features ({len(cols)}): {cols}")
        featured_data = engineer.create_binned_features(featured_data)
        cols = list(featured_data.columns)
        print(f"After binned features ({len(cols)}): {cols}")

        # Verify new features were created
        msg = (
            f"Expected more features, got {len(featured_data.columns)} "
            f"vs {len(processed_data.columns)}"
        )
        assert len(featured_data.columns) >= len(processed_data.columns), msg
...
class TestModelPipelineIntegration:
    ...
    def test_segmentation_to_prediction_integration(
        self, temp_dir, sample_featured_data
    ):
        ...
        assert len(segment_profiles) == optimal_k
        assert all(
            "churn_rate" in profile for profile in segment_profiles.values()
        )
        ...
        predictor.save_model(
            baseline_model, "LogisticRegression", baseline_metrics
        )
...
class TestAPIIntegration:
    ...
    def test_model_to_api_integration(self, temp_dir):
        ...
        try:
            ...
            try:
                manager.predict_single(customer_data)
...
class TestPerformanceIntegration:
    ...
    def test_pipeline_performance(self):
        ...
        try:
            ...
            clean_data = cleaner.remove_pii(interim_data)
            clean_data = cleaner.handle_missing_values(clean_data)
            result = cleaner.create_preprocessing_pipeline(clean_data)
            pipeline, processed_data = result
            cleaner.save_processed_data(processed_data)
            ...
            X, y = predictor.load_and_prepare_data()
            X_train, X_test, y_train, y_test = predictor.split_data(X, y)
            baseline_result = predictor.train_baseline_model(
                X_train, y_train, X_test, y_test
            )
            baseline_model, baseline_metrics = baseline_result
            ...
            # Lower threshold for random test data
            acc = baseline_metrics["accuracy"]
            assert acc > 0.4, f"Baseline accuracy {acc:.2f} too low"
            acc = baseline_metrics["accuracy"]
