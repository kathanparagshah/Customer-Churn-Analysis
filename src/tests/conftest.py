...
Shared fixtures and configuration for all test modules in the project.
        ...
        data = pd.DataFrame(
            {
                ...
                "EstimatedSalary": np.random.uniform(
                    11.58, 199992.48, n_customers
                ),
                "Exited": np.random.choice(
                    [0, 1], n_customers, p=[1 - churn_rate, churn_rate]
                ),
            }
        )
...
class PerformanceTimer:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.max_time and self.elapsed_time > self.max_time:
            pytest.fail(
                f"Operation took {self.elapsed_time:.2f}s, "
                f"expected < {self.max_time}s"
            )
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.max_memory_mb and memory_increase > self.max_memory_mb:
            pytest.fail(
                ...
                f"expected < {self.max_memory_mb}MB"
            )
