...
Comprehensive tests for data loading, cleaning, and feature engineering.
...
from data.clean_data import DataCleaner  # noqa: E402
from data.load_data import DataLoader  # noqa: E402
from features.create_features import FeatureEngineer  # noqa: E402
...
class TestDataLoader:
    ...
    def test_perform_sanity_checks_invalid_ranges(
        self, data_loader, sample_data
    ):
    ...
    def test_validate_data_quality_duplicate_rows(
        self, data_loader, sample_data
    ):
        """Test data quality validation with duplicate rows."""
        # Add duplicate rows
        duplicate_data = pd.concat(
            [sample_data, sample_data.iloc[:2]], ignore_index=True
        )
    ...
    def test_validate_data_quality_missing_values(
        self, data_loader, sample_data
    ):
...
class TestDataCleaner:
    ...
    def test_init(self, data_cleaner, temp_dir):
        ...
        assert (
            data_cleaner.processed_data_dir == temp_dir / "data" / "processed"
        )
    ...
    def test_handle_missing_values_numeric(
        self, data_cleaner, sample_raw_data
    ):
    ...
    def test_encode_categorical_variables(self, data_cleaner, sample_raw_data):
        ...
        geography_cols = [
            col for col in processed_data.columns if "Geography" in col
        ]
        gender_cols = [
            col for col in processed_data.columns if "Gender" in col
        ]
    ...
    def test_save_processed_data(
        self, data_cleaner, sample_raw_data, temp_dir
    ):
    ...
    def test_analyze_missing_values(
        self, data_cleaner, sample_data_with_missing
    ):
        """Test missing value analysis."""
        analysis = data_cleaner.analyze_missing_values(
    ...
    def test_handle_missing_values(
        ...
        cleaned_data = data_cleaner.handle_missing_values(
            sample_data_with_missing
        )
    ...
    def test_validate_processed_data_edge_cases(self, data_cleaner):
        """Test processed data validation edge cases."""
...
class TestFeatureEngineer:
    ...
    def test_init(self, feature_engineer, temp_dir):
        ...
        assert (
            feature_engineer.processed_dir == temp_dir / "data" / "processed"
        )

    def test_create_ratio_features(
        self, feature_engineer, sample_processed_data
    ):
        """Test ratio feature creation."""
        enhanced_data = feature_engineer.create_ratio_features(
        ...
        # Check calculations
        expected_balance_to_salary = sample_processed_data["Balance"] / (
            sample_processed_data["EstimatedSalary"] + 1
        )
    ...
    def test_create_binned_features(
        ...
        enhanced_data = feature_engineer.create_binned_features(
            sample_processed_data
        )
    ...
    def test_create_behavioral_features(
        self, feature_engineer, sample_processed_data
    ):
        """Test behavioral feature creation."""
        enhanced_data = feature_engineer.create_behavioral_features(
            sample_processed_data
        )
    ...
    def test_create_age_groups(self, feature_engineer, sample_processed_data):
        ...
        enhanced_data = feature_engineer.create_age_groups(
            sample_processed_data
        )
        ...
        age_group_cols = [
            ...
            if col.startswith("AgeGroup_") and not col.endswith("_num")
        ]
        ...
        assert (
            age_group_sum == 1
        ).all()  # Each row should have exactly one age group

    def test_create_balance_categories(
        self, feature_engineer, sample_processed_data
    ):
        """Test creation of balance category features."""
        enhanced_data = feature_engineer.create_balance_categories(
            sample_processed_data
        )
        ...
        balance_cat_cols = [
            col
            for col in enhanced_data.columns
            if col.startswith("BalanceCategory_") and not col.endswith("_num")
        ]
    ...
    def test_create_interaction_features(
        ...
        enhanced_data = feature_engineer.create_interaction_features(
            sample_processed_data
        )
        ...
        assert (
            enhanced_data["Age_x_Tenure"].iloc[0] == 42 * 2
        )  # Age 42, Tenure 2
    ...
    def test_create_polynomial_features(
        ...
        enhanced_data = feature_engineer.create_polynomial_features(
            sample_processed_data
        )
    ...
    def test_select_features_univariate(
        ...
        feature_cols = [
            col for col in selected_data.columns if col != "Exited"
        ]
    ...
    def test_select_features_importance(
        self, feature_engineer, sample_processed_data
    ):
        ...
        # Select features
        selected_data, importance_scores = (
            feature_engineer.select_features_importance(
    ...
    def test_create_pca_features(
        self, feature_engineer, sample_processed_data
    ):
        """Test feature creation methods."""
        # Test ratio features
        ratio_data = feature_engineer.create_ratio_features(
            sample_processed_data
        )
        assert ratio_data.shape[1] >= sample_processed_data.shape[1]

        # Test binned features
        binned_data = feature_engineer.create_binned_features(
            sample_processed_data
        )
        assert binned_data.shape[1] >= sample_processed_data.shape[1]

        # Test behavioral features
        behavioral_data = feature_engineer.create_behavioral_features(
            sample_processed_data
        )
    ...
    def test_validate_features(self, feature_engineer, sample_processed_data):
        ...
        assert (
            not np.isinf(enhanced_data.select_dtypes(include=[np.number]))
            .any()
            .any()
        )

    def test_save_features(
        self, feature_engineer, sample_processed_data, temp_dir
    ):
        ...
        # Verify file exists
        output_path = (
            feature_engineer.processed_data_dir / "churn_features.parquet"
        )
        assert output_path.exists()
        loaded_data = pd.read_parquet(output_path)
        assert loaded_data.shape == enhanced_data.shape

    def test_full_feature_engineering_pipeline(
        self, feature_engineer, sample_processed_data, temp_dir
    ):
        ...
        features_file = (
            feature_engineer.processed_data_dir / "churn_features_all.parquet"
        )
        selected_file = (
            feature_engineer.processed_data_dir
            / "churn_features_selected.parquet"
        )
    ...
    def test_create_statistical_features(
        ...
        enhanced_data = feature_engineer.create_statistical_features(
            sample_processed_data
        )
        ...
        zscore_features = [
            col for col in enhanced_data.columns if col.endswith("_zscore")
        ]
    ...
    def test_calculate_feature_importance(
        ...
        # Calculate importance - returns dict with feature names as keys
        importance_results = feature_engineer.calculate_feature_importance(
            enhanced_data
        )
        # Verify importance results are valid
        assert importance_results is not None

        # Check results structure - should be dict with feature names/scores
...
class TestIntegration:
    ...
    def test_complete_pipeline(self, temp_dir, sample_raw_data):
        ...
        importance_results = engineer.calculate_feature_importance(
            enhanced_data
        )
        # Verify importance results are valid
        assert isinstance(importance_results, dict)
        top_features = engineer.select_top_features(
            enhanced_data, n_features=10
        )

        # Verify final results
        assert len(enhanced_data) == len(sample_raw_data)
        # Check that we have the requested number of features
        # plus ID and target columns
        excluded_cols = ["RowNumber", "CustomerId", "Exited"]
        feature_only_count = len(
            [f for f in top_features if f not in excluded_cols]
        )
        ...
        orig_rows = len(sample_raw_data)
        orig_cols = len(sample_raw_data.columns)
        print(f"   - Original data: {orig_rows} rows, {orig_cols} columns")
        enh_rows = len(enhanced_data)
        enh_cols = len(enhanced_data.columns)
        print(f"   - Enhanced data: {enh_rows} rows, {enh_cols} columns")
