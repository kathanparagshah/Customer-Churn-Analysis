...
try:
    from deployment.app import CustomerData, ModelManager, app
    ...
    ModelManager = None
    print(
        f"Warning: API module not found. Import error: {e}. "
...
class TestCustomerDataValidation:
    ...
    @pytest.mark.skipif(
        CustomerData is None, reason="API module not available"
    )
    ...
    def test_invalid_credit_score(self, valid_customer_data):
        ...
        with pytest.raises(
            ValueError, match="Credit score must be between 300 and 850"
        ):
    ...
    def test_invalid_geography(self, valid_customer_data):
        ...
        with pytest.raises(
            ValueError,
            match="Geography must be one of: France, Germany, " "Spain",
        ):
            CustomerData(**invalid_data)
    ...
    def test_invalid_gender(self, valid_customer_data):
        ...
        with pytest.raises(
            ValueError, match="Gender must be either Male or Female"
        ):
            CustomerData(**invalid_data)
    ...
    def test_invalid_tenure(self, valid_customer_data):
        ...
        with pytest.raises(
            ValueError, match="Tenure must be between 0 and 10"
        ):
            CustomerData(**invalid_data)
    ...
    def test_invalid_num_products(self, valid_customer_data):
        ...
        with pytest.raises(
            ValueError, match="Number of products must be between 1 and 4"
        ):
            CustomerData(**invalid_data)
    ...
    def test_invalid_estimated_salary(self, valid_customer_data):
        ...
        with pytest.raises(
            ValueError, match="Estimated salary must be positive"
        ):
            CustomerData(**invalid_data)
...
class TestModelManager:
    ...
    def mock_model_package(self, temp_dir):
        ...
        # Define feature names that match the actual deployment
        # structure (11 features)
        feature_names = [
            "CreditScore",
            "Geography_Germany",
            "Geography_Spain",
        ...
        X_sample = np.random.randn(100, 11)  # 11 features to match
        # expected_features
    ...
    @pytest.mark.skipif(
        ModelManager is None, reason="API module not available"
    )
    def test_model_manager_init(self):
        """Test ModelManager initialization."""
        manager = ModelManager()

        # ModelManager may auto-load model if it exists, so check if loaded
...
class TestAPIEndpoints:
    ...
    def test_root_endpoint(self, client):
        """Test root endpoint."""
        response = client.get("/")
        assert response.status_code == 200  # Model is loaded by integration
    ...
    def test_health_endpoint(self, client):
        ...
        with patch("app.services.model_manager.model_manager") as mock_mgr:
            # Mock the model manager's properties and methods
            mock_mgr.is_loaded = True
            mock_mgr.version = "1.0.0"
            mock_mgr.get_health_status.return_value = {
                "uptime": "0:01:23",
    ...
    def test_predict_endpoint_success(self, client, valid_customer_data):
        ...
        with patch("app.services.model_manager.model_manager") as mock_mgr:
            ...
            mock_mgr.predict_single.return_value = {
    ...
    def test_batch_predict_endpoint_success(self, client, valid_customer_data):
        ...
        with patch("app.services.model_manager.model_manager") as mock_mgr:
            ...
            mock_mgr.predict_batch.return_value = [
                {
    ...
    def test_batch_predict_endpoint_too_large(
        self, client, valid_customer_data
    ):
    ...
    def test_batch_predict_endpoint_empty(self, client):
        ...
        # Empty batch should return validation error (422) as per schema
        assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
        data = response.json()
        assert "detail" in data
    ...
    def test_cors_headers(self, client):
        ...
        with patch("app.services.model_manager.model_manager") as mock_mgr:
            ...
            mock_mgr.get_uptime.return_value = "0:01:23"
...
def test_prediction_model_exception_standalone():
    ...
    with patch("app.services.model_manager.model_manager") as mock_mgr:
        ...
        mock_mgr.predict_single.side_effect = Exception(
...
class TestErrorHandling:
    ...
    def test_prediction_error_handling(self, client):
        ...
        with patch("app.services.model_manager.model_manager") as mock_mgr:
            ...
            mock_mgr.predict_single.side_effect = ValueError(
                "Model not loaded"
            )
    ...
    # Note: test_prediction_model_exception moved to standalone function
    # above the class
...
class TestPerformanceAndLoad:
    ...
    def test_large_batch_performance(self, client, valid_customer_data):
        ...
        with patch("app.services.model_manager.model_manager") as mock_mgr:
            ...
            mock_mgr.predict_batch.return_value = mock_predictions

            # Create large batch
            large_batch = {"customers": [valid_customer_data] * 100}
            ...
            assert response.status_code == status.HTTP_200_OK
            assert response_time < 5.0  # Should process 100 customers
...
class TestAPIEndpointsComprehensive:
    ...
    def test_predict_endpoint_success_comprehensive(
        self, client, valid_customer_data
    ):
    ...
    def test_batch_predict_endpoint_success_comprehensive(
        self, client, valid_customer_data
    ):
        """Test successful batch prediction endpoint with comprehensive
        checks."""
        with patch("app.services.model_manager.model_manager") as mock_mgr:
    ...
    def test_batch_predict_too_many_customers(
        self, client, valid_customer_data
    ):
        """Test batch prediction with too many customers."""
        # Create batch with more than allowed limit (assuming 1000 is limit)
        large_batch = {"customers": [valid_customer_data] * 1001}
